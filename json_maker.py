...
def prejson(tableaulog):  # permet de donner une clé aux valeur obtenues grâce à parsing()
    ...
    for ligne in tableaulog:
        lignejson = {}
        # Je regarde si le premier champ est une adresse IP ou autre chose
        if deconcat.whatis(re.search('([0-9]{1,3}\.){3}[0-9]{1,3}',
                                     ligne[0])) != "-":
        ...
        request_champs = re.search('"([^ ]*) ([^ ]*) ([^"]*)"', ligne[
        ...
        lignejson['system_agent'] = deconcat.whatis(re.search('Linux|Macintosh|Windows|iPhone|SAMSUNG', ligne[
            8]))  # le système du client (il n'y a pas que ces 4 système
...
tableau_pre_json = prejson(deconcat.parsing1('apache_logs'))
...
def json_maker(pre_json):
    with open('log.json',
        ...
        print(f"J'ai écrit {len(pre_json)} de ligne dans log.json")
        json.dump(pre_json, fichier_json, indent=4)
